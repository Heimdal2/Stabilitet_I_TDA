...
\end{lemma}
\bevis{
  Vi viser dette for intervallmodulen $K^I$ for et
  intervall $I\subset\Rbb$ og teoremet gjelder for enhver
  intervalldekomponering. Vi har at
  \[S^{K^I,\delta} = 
  \begin{cases}
    \id_K,\quad \text{Hvis $t\in I\cap I(\delta)$}\\
    0,\quad \text{ellers}
  \end{cases}
  \]
  Dette faktoriseres som $K^I\xepito{q} K^{I\cap
  I(\delta)}\xmonicto{i} I(\delta)$ med
  \[q_t =
  \begin{cases}
    K,\quad \text{Hvis $s\in I\cap I(\delta)$}\\
    0,\quad \text{ellers}
  \end{cases}
  \]
  og $i$ er gitt ved punktvis inklusjon. Vi har at $I\cap
  I(\delta)$ og $I$ samfaller under altså sender $\chi(q)$
  $I$ til $I\cap I(\delta)$. Vi har også at $I\cap
  I(\delta)$ samfaller $I(\delta)$ over, som betyr at
  $\chi(i)$ sender $I\cap I(\delta)$ til $I(\delta)$.
  Dette betyr at $\chi(S^{K^I,\delta})$ sender $I$ til
  $I(\delta)$ som $S^{\Bcal(K^I),\delta}$ også gjør.
}

Med dette lemmaet får vi beviset for
\bref{teorem}{trm:IKT} som er gitt i \citep{Bauer2020}
